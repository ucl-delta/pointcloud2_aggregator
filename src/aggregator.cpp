    ...
    publish_frequency(10)
{
    ...
    this->declare_parameter("time_window", 0.5);
    ...
    topic_name.append("_aggregated");
    this->aggregator_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>(topic_name, this->publish_frequency);
    ...
    RCLCPP_INFO(this->get_logger(), "Pointcloud2 Aggregator Initialised");
};
...
void Pointcloud2_Aggregator::pointcloud2_sub_callback(const sensor_msgs::msg::PointCloud2& msg) 
{
    ...
    if(!this->pointcloud_template) {
        ...
        if (
          ...
          this->pointcloud_template->row_step != shared_msg->row_step  
        ) {
            RCLCPP_ERROR(this->get_logger(), "Subsequent pointcloud msgs inconsistent");
        } 
    }
